...
import json
import requests
import pydeck as pdk  # para mapa/route render
...
def compute_volume_m3(length_cm: float, width_cm: float, height_cm: float, count: int) -> float:
    # L * W * H en cm^3 -> m^3 y multiplicado por cantidad
    return (length_cm * width_cm * height_cm) / 1e6 * count

def compute_density_kg_m3(weight_tons: float, volume_m3: float) -> float | None:
    # weight_tons -> kg
...
def estimate_pallet_positions(count: int, stackable: bool) -> int:
    # Regla simple: si apilable, 2 por posición; si no, 1 por posición (techo)
...
# =========================
#   Ensure default session state for inputs + suggestions state
# =========================
...
with st.form("quote"):
    ...
    with c1:
        ...
        # --- Origin/Destination con sugerencias vía backend /v1/geo/suggest ---
        origin = st.text_input("Origin", value=st.session_state.get("origin","Jebel Ali Port"), key="origin")
        if st.button("Suggest Origin"):
            try:
                r = requests.get(f"{API_URL}/v1/geo/suggest", params={"q": st.session_state["origin"]}, timeout=20)
                r.raise_for_status()
        ...
        if st.button("Suggest Destination"):
            try:
                r = requests.get(f"{API_URL}/v1/geo/suggest", params={"q": st.session_state["destination"]}, timeout=20)
                r.raise_for_status()
    ...
    # ---- Pallets expander (nuevo) ----
    with st.expander("Pallets"):
        col1, col2, col3, col4 = st.columns(4)
        pallet_length_cm = col1.number_input(
            "Length (cm)", min_value=1.0, value=float(st.session_state.get("pallet_length_cm", 120.0)), key="pallet_length_cm"
        )
        pallet_width_cm = col2.number_input(
            "Width (cm)", min_value=1.0, value=float(st.session_state.get("pallet_width_cm", 100.0)), key="pallet_width_cm"
        )
        pallet_height_cm = col3.number_input(
            "Height (cm)", min_value=1.0, value=float(st.session_state.get("pallet_height_cm", 150.0)), key="pallet_height_cm"
        )
        pallet_count = col4.number_input(
            "Count", min_value=1, value=int(st.session_state.get("pallet_count", 18)), step=1, key="pallet_count"
        )
        pallet_stackable = st.checkbox("Stackable", value=bool(st.session_state.get("pallet_stackable", True)), key="pallet_stackable")

    submitted = st.form_submit_button("Predict")


# =========================
#   Route features (Mapbox) y trazado de ruta
# =========================
st.markdown("—")
...
with colC:
    st.caption("Use the buttons to pre-fill features or visualize the route. You can still edit values before predicting.")

def _extract_coords_from_route(route_json):
    """
    Acepta dos formatos:
      1) route['legs'][i]['coordinates'] -> [[lon,lat], ...]
      2) route['legs'][i]['geometry_polyline'] -> str (requiere 'polyline' lib)
    Devuelve lista de [lon, lat].
    """
    coords = []
    try:
    ...
    try:
        ...
        if not params["origin"] or not params["destination"]:
            ...
            if coords:
                ...
                view_state = pdk.ViewState(latitude=center[1], longitude=center[0], zoom=9, bearing=0, pitch=0)

                # Si quieres basemap de Mapbox (opcional):
...
try:
    ...
    _dens = compute_density_kg_m3(
        float(st.session_state.get("load_weight_tons", 3.2)),
        _vol_m3
    )
...
if submitted:
    payload = {
        ...
        # ---- NUEVO: estructura pallets (alineada con la API) ----
        "pallets": {
